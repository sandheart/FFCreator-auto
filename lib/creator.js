...
const { Application, Loader, settings, destroyAndCleanAllCache } = require('inkpaint-auto');
...
class FFCreator extends FFCon {
  ...
  createApp() {
    ...
    const app = Pool.get(key, () => new Application(width, height, appOptions));
    ...
    if (this.children) {
      ...
      for (let i = 1; i <= ffsceneList.length; i++) {
        ...
        if (i < ffsceneList.length) {
          ...
          totalDurationOfTranslations =
            (totalDurationOfTranslations * 1000000 +
              ffsceneList[i - 1].transition.duration * 1000000) /
            1000000;
        }
